...
upload_file= st.file_uploader("Titanic.csv 파일을 직접 업로드할 수 있습니다(선택사항)" ,type = "csv")
...
if df is not None : 
    ...
    with col1 : 
        st.metric("행 개수", f"{df.shape[0]}개") #행=0
    with col2 : 
        st.metric("열 개수", f"{df.shape[1]}개") #열=1 
    ...
    # io.StringIO() 라는 "메모리 위에 가짜 파일"에 결과를 받아낸 뒤 그 내용을 text로 보여준다.4T5YH6H6UU
